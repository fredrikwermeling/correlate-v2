...
class CorrelationExplorer {
    ...
    setupUI() {
        ...
        document.getElementById('closeCorrAnalysis')?.addEventListener('click', () => {
            document.getElementById('corrAnalysisModal').style.display = 'none';
        });
        document.getElementById('corrAnalysisModal')?.addEventListener('click', (e) => {
            if (e.target.id === 'corrAnalysisModal') {
                document.getElementById('corrAnalysisModal').style.display = 'none';
            }
        });
        document.getElementById('caViewTissue')?.addEventListener('click', () => {
            this.switchCorrAnalysisView('tissue');
        });
        document.getElementById('caViewHotspot')?.addEventListener('click', () => {
...
`;
        ...
        if (tab === 'tissue') {
            ...
            let html = `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                ...
                <div style="display:flex;gap:4px;align-items:center;">
                    <button onclick="app.enrichrGateMutations('A')" style="background:#e8910c;color:white;border:none;border-radius:4px;padding:3px 10px;font-size:11px;cursor:pointer;white-space:nowrap;" title="Submit top genes enriched in Gate A to Enrichr">Enrichr A ↗</button>
    ...
    showGateGenePlot(gene, type) {
        ...
        if (type === 'mutation') {
            ...
            const geGateAIds = new Set(gateA.map(d => d.cellLineId));
            const geGateBIds = new Set(gateB.map(d => d.cellLineId));
            const geGateANameMap = new Map(gateA.map(d => [d.cellLineId, d.cellLineName || d.cellLineId]));
            const geGateBNameMap = new Map(gateB.map(d => [d.cellLineId, d.cellLineName || d.cellLineId]));
            const geValsA = [], geNamesA = [], geValsB = [], geNamesB = [];
            for (let j = 0; j < this.nCellLines; j++) {
                const val = this.geneEffects[geneIdx * this.nCellLines + j];
                if (isNaN(val)) continue;
                const cl = this.metadata.cellLines[j];
                if (geGateAIds.has(cl)) { geValsA.push(val); geNamesA.push(geGateANameMap.get(cl) || cl); }
                else if (geGateBIds.has(cl)) { geValsB.push(val); geNamesB.push(geGateBNameMap.get(cl) || cl); }
            }

            const traces = [
                {
                    y: geValsA,
                    x: geValsA.map(() => 'Gate A'),
                    type: 'box',
                    name: `Gate A (n=${geValsA.length})`,
                    marker: { color: '#2563eb' },
                    boxpoints: 'all',
                    jitter: 0.4,
                    pointpos: 0,
                    boxmean: true,
                    text: geNamesA,
                    hovertemplate: '%{text}<br>GE: %{y:.3f}<extra></extra>'
                },
                {
                    y: geValsB,
                    x: geValsB.map(() => 'Gate B'),
                    type: 'box',
                    name: `Gate B (n=${geValsB.length})`,
                    marker: { color: '#dc2626' },
                    boxpoints: 'all',
                    jitter: 0.4,
                    pointpos: 0,
            ...
            const exprValsA = [], exprNamesA = [], exprValsB = [], exprNamesB = [];
            for (const d of gateA) {
                const exprIdx = exprCellLineIndex.get(d.cellLineId);
                if (exprIdx !== undefined) {
                    const val = this.expressionData[exprGeneIdx * nExprCellLines + exprIdx];
                    if (!isNaN(val)) { exprValsA.push(val); exprNamesA.push(d.cellLineName || d.cellLineId); }
                }
            }
            for (const d of gateB) {
                const exprIdx = exprCellLineIndex.get(d.cellLineId);
                if (exprIdx !== undefined) {
                    const val = this.expressionData[exprGeneIdx * nExprCellLines + exprIdx];
                    if (!isNaN(val)) { exprValsB.push(val); exprNamesB.push(d.cellLineName || d.cellLineId); }
                }
            }
            ...
            const traces = [
                {
                    y: exprValsA,
                    x: exprValsA.map(() => 'Gate A'),
                    type: 'box',
                    name: `Gate A (n=${exprValsA.length})`,
                    marker: { color: '#2563eb' },
                    boxpoints: 'all',
                    jitter: 0.4,
                    pointpos: 0,
                    boxmean: true,
                    text: exprNamesA,
                    hovertemplate: '%{text}<br>Expr: %{y:.3f}<extra></extra>'
                },
                {
                    y: exprValsB,
                    x: exprValsB.map(() => 'Gate B'),
                    type: 'box',
                    name: `Gate B (n=${exprValsB.length})`,
                    marker: { color: '#dc2626' },
                    boxpoints: 'all',
                    jitter: 0.4,
                    pointpos: 0,
    ...
    _handleGEGateShapeRelayout(relayoutData) {
        const keys = Object.keys(relayoutData);
        const hasShapeKeys = keys.some(k => k.startsWith('shapes'));
        if (!hasShapeKeys) return;
        ...
        // Detect newly drawn shape
        if (this._geGateSelecting && currentShapes.length > 0) {
            const newShape = currentShapes[currentShapes.length - 1];
            const gate = this._geGateSelecting;
            const { x0, x1, y0, y1 } = newShape;
            const cells = this._computeGEGateCells(x0, x1, y0, y1);

            if (gate === 'A') {
                if (this._geGateAShapeIndex != null && this._geGateAShapeIndex !== currentShapes.length - 1) {
                    currentShapes.splice(this._geGateAShapeIndex, 1);
                    if (this._geGateBShapeIndex != null && this._geGateBShapeIndex > this._geGateAShapeIndex) {
                        this._geGateBShapeIndex--;
                    }
                }
                this._geGateAShapeIndex = currentShapes.length - 1;
                this._geGateAShape = { x0, x1, y0, y1 };
                this._geGateA = cells;

                currentShapes[this._geGateAShapeIndex] = {
                    ...currentShapes[this._geGateAShapeIndex],
                    line: { color: '#2563eb', width: 2 },
                    fillcolor: 'rgba(37,99,235,0.1)',
                    opacity: 1,
                ...
                if (this._geGateBShapeIndex != null && this._geGateBShapeIndex !== currentShapes.length - 1) {
                    ...
                    if (this._geGateAShapeIndex != null && this._geGateAShapeIndex > this._geGateBShapeIndex) {
                        this._geGateAShapeIndex--;
                    }
                }
                this._geGateBShapeIndex = currentShapes.length - 1;
                this._geGateBShape = { x0, x1, y0, y1 };
                this._geGateB = cells;

                currentShapes[this._geGateBShapeIndex] = {
                    ...currentShapes[this._geGateBShapeIndex],
                    line: { color: '#dc2626', width: 2 },
                    fillcolor: 'rgba(220,38,38,0.1)',
                    opacity: 1,
        ...
        // Handle editing of existing shapes (drag/resize)
        let gateAUpdated = false, gateBUpdated = false;
        for (const key of keys) {
            const m = key.match(/^shapes\[(\d+)\]\.(x0|x1|y0|y1)$/);
            if (m) {
                const idx = parseInt(m[1]);
                const prop = m[2];
                if (idx === this._geGateAShapeIndex && this._geGateAShape) {
                    this._geGateAShape[prop] = relayoutData[key];
                    gateAUpdated = true;
                } else if (idx === this._geGateBShapeIndex && this._geGateBShape) {
                    this._geGateBShape[prop] = relayoutData[key];
                    gateBUpdated = true;
                }
            }
        }
        if (gateAUpdated && this._geGateAShape) {
            const { x0, x1, y0, y1 } = this._geGateAShape;
            this._geGateA = this._computeGEGateCells(x0, x1, y0, y1);
            document.getElementById('geSetGateABtn').textContent = `Gate A (n=${this._geGateA.length})`;
        }
        if (gateBUpdated && this._geGateBShape) {
            const { x0, x1, y0, y1 } = this._geGateBShape;
            this._geGateB = this._computeGEGateCells(x0, x1, y0, y1);
            document.getElementById('geSetGateBBtn').textContent = `Gate B (n=${this._geGateB.length})`;
        }
        if (gateAUpdated || gateBUpdated) {
            this.updateGEGateOverlay();
    ...
    _computeGEGateCells(x0, x1, y0, y1) {
    ...
    clearGEGates() {
        ...
        this._geGateAShape = null;
        this._geGateBShape = null;
        this._geGateAShapeIndex = null;
        this._geGateBShapeIndex = null;
        ...
        if (el('geGateStatus')) { el('geGateStatus').textContent = 'Draw rectangles on plot to define gates'; el('geGateStatus').style.color = '#6b7280'; }
    ...
    renderGEGateTab(tab) {
        ...
        if (tab === 'tissue') {
            ...
            let html = `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                <p style="font-size:10px;color:#6b7280;margin:0;">Hotspot: ${nHotspot} genes, Damaging: ${nDamaging} genes. Chi-squared test for enrichment. Top 100 shown.</p>
                <div style="display:flex;gap:4px;align-items:center;">
                    <button onclick="app.enrichrGEGateMutations('A')" style="background:#e8910c;color:white;border:none;border-radius:4px;padding:3px 10px;font-size:11px;cursor:pointer;white-space:nowrap;" title="Submit top genes enriched in Gate A to Enrichr">Enrichr A ↗</button>
    ...
    enrichrGateMutations(gate = 'A') {
        ...
        if (genes.length < 2) {
            this.showCopyNotification(`Need at least 2 genes enriched in Gate ${gate} for Enrichr`);
            return;
        }
    ...
    filterGateGeneTable(term) {
        ...
        rows.forEach(row => {
            const gene = row.querySelector('td')?.textContent?.toLowerCase() || '';
            row.style.display = gene.includes(lowerTerm) ? '' : 'none';
        });
    }

    filterGEGateGeneTable(term) {
        const container = document.getElementById('geGateCompareContent');
        if (!container) return;
        const table = container.querySelector('table');
        if (!table) return;
    ...
    switchCorrAnalysisView(view) {
        ...
        [tissueBtn, hotspotBtn].forEach(btn => {
        ...
        if (view === 'tissue') {
            ...
            if (explanation) explanation.textContent = "Pearson correlation per cancer type. p-value: Fisher z-test vs all other cell lines.";
            ...
            if (explanation) explanation.textContent = "Shows WT (blue) vs mutated (red) correlation. p-value: Fisher z-test comparing the two correlations.";
            this.renderCorrAnalysisByHotspot();
        }
    }

    renderCorrAnalysisByTissue() {
        const d = this._corrAnalysisData;
        if (!d) return;
        ...
        Object.entries(groups).forEach(([tissue, pts]) => {
            if (pts.length >= 3) {
                ...
                // p-value: Fisher z-test vs all other cell lines
                const otherPts = data.filter(p => (p.lineage || 'Unknown') !== tissue);
                let pValue = 1;
        ...
        // Sort by correlation (like gene effect sorts by mean)
        stats.sort((a, b) => a.correlation - b.correlation);
        this._caCurrentStats = stats;

        if (stats.length === 0) {
            document.getElementById('corrAnalysisTissuePlot').innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:300px;color:#6b7280;">No tissues with ≥3 cell lines</div>';
            document.getElementById('corrAnalysisTableBody').innerHTML = '';
            return;
        }
        ...
        Plotly.newPlot('corrAnalysisTissuePlot', traces, layout, { responsive: true });

        // Table — sorted by p-value for table display
        const tableStats = stats.map(s => ({
            group: s.group, n: s.n, correlation: s.correlation, slope: s.slope, pValue: s.pValue
        }));
        tableStats.sort((a, b) => a.pValue - b.pValue);
        this._caCurrentStats = tableStats;
        this.renderCATable(tableStats, 'tissue');
    }
    ...
    renderCorrAnalysisByHotspot() {
        ...
        if (!this.mutations?.genes || this.mutations.genes.length === 0) {
            document.getElementById('corrAnalysisHotspotPlot').innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:300px;color:#6b7280;">No hotspot mutation data available</div>';
            document.getElementById('corrAnalysisTableBody').innerHTML = '';
            return;
        }
        ...
        this.mutations.genes.forEach(hotspotGene => {
            ...
            data.forEach(p => {
                const level = mutData[p.cellLineId] || 0;
                if (level === 0) wtPts.push(p);
                else mutPts.push(p);
            });

            if (wtPts.length >= 3 && mutPts.length >= 3) {
                const wtCorr = this.pearsonWithSlope(wtPts.map(p => p.x), wtPts.map(p => p.y));
        ...
        if (hotspotStats.length === 0) {
            document.getElementById('corrAnalysisHotspotPlot').innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:300px;color:#6b7280;">No hotspots with ≥3 WT and ≥3 mutated cell lines</div>';
            document.getElementById('corrAnalysisTableBody').innerHTML = '';
            return;
        }
        ...
        Plotly.newPlot('corrAnalysisHotspotPlot', traces, layout, { responsive: true });

        // Table — sorted by p-value
        const tableStats = hotspotStats.map(s => ({
            group: s.group, n0: s.n0, nMut: s.nMut, r0: s.r0, rMut: s.rMut, diff: s.diff, pValue: s.pValue
        }));
        tableStats.sort((a, b) => a.pValue - b.pValue);
        this.renderCATable(tableStats, 'hotspot');
    }
    ...
    renderCATableBody(stats, mode) {
        ...
        tbody.querySelectorAll('.clickable-row').forEach(row => {
            row.addEventListener('click', () => {
                this.showCADetailedView(row.dataset.group);
            });
        });
    }
    ...
    showCADetailedView(group) {
        const d = this._corrAnalysisData;
        if (!d) return;

        this._caDetailedView = group;
        document.getElementById('caShowAllBtn').style.display = '';

        const mode = this._caView;
        let plotId, traces, layout;

        if (mode === 'tissue') {
            // Show scatter of this tissue's cell lines
            const pts = d.data.filter(p => (p.lineage || 'Unknown') === group);
            ...
            plotId = 'corrAnalysisHotspotPlot';
            traces = [
                {
                    type: 'scatter', mode: 'markers',
    ...
    downloadCorrAnalysisCSV() {
        const d = this._corrAnalysisData;
        const stats = this._caCurrentStats;
        if (!d || !stats) return;
    ...
    updateClbFilterCounts() {
        ...
        // Helper: filter cell lines by all filters EXCEPT the excluded one
    ...
    exportCellLineBrowserCSV(mode) {
        ...
        if (mode === 'minimal') {
            ...
            for (let c = 0; c < clIndices.length; c++) {
                ...
                // Compute gene effects for this cell line
                const geneVals = [];
                for (let g = 0; g < this.nGenes; g++) {
...
<image href="${genePlotSvgStr}" width="${rightW}" height="${rightH}" preserveAspectRatio="xMidYMid meet"/>
    ...
    _resetUmap() {
        this._clbUmapData = null;
        this._clbUmapAllScores = null;
        this._clbUmapAllExplained = null;
    ...
    async umapFromGateA() {
        ...
        try {
            ...
            if (method === 'pca') {
                ...
                const nc = pca.scores[0].length;
                this._clbUmapAllScores = pca.scores;
                this._clbUmapAllExplained = pca.explained;
                this._clbUmapAllLoadings = pca.loadings;
                ...
                for (let k = 0; k < nc; k++) {
                    ...
                    for (let g = 0; g < geneNames.length; g++) {
                        pcaLoadings[pcKey].push({ gene: geneNames[g], loading: pca.loadings[g][k] });
                    }
                    pcaLoadings[pcKey].sort((a, b) => Math.abs(b.loading) - Math.abs(a.loading));
                }
                this._populateComponentSelectors(nc, 'PC', pca.explained);
            } else {
                const nNeighbors = Math.min(15, Math.floor(nCL / 3));
                const UMAPClass = (typeof UMAP === 'function') ? UMAP : UMAP.UMAP;
                const nDims = nCL >= 30 ? 3 : 2;
                const umap = new UMAPClass({ nNeighbors: Math.max(5, nNeighbors), minDist: 0.3, nComponents: nDims, spread: 1.5 });
                const embedding = umap.fit(matrix);
                this._clbUmapAllScores = embedding;
                this._clbUmapAllExplained = null;
                this._clbUmapAllLoadings = null;
                x = embedding.map(e => e[0]);
                y = embedding.map(e => e[1]);
                axisLabels = ['UMAP 1', 'UMAP 2'];
                this._populateComponentSelectors(nDims, 'UMAP', null);
            }
    ...
    async runCLBUmap() {
        ...
        try {
            ...
            if (method === 'pca') {
                const pca = this._computePCA(matrix, 5);
                const nc = pca.scores[0].length;
                // Store all scores for component switching
                this._clbUmapAllScores = pca.scores;
                this._clbUmapAllExplained = pca.explained;
                this._clbUmapAllLoadings = pca.loadings;
                x = pca.scores.map(s => s[0]);
                ...
                const umap = new UMAPClass({ nNeighbors: Math.max(5, nNeighbors), minDist: 0.3, nComponents: nDims, spread: 1.5 });
                const embedding = umap.fit(matrix);
                this._clbUmapAllScores = embedding; // [n][nDims]
                this._clbUmapAllExplained = null;
                this._clbUmapAllLoadings = null;
                x = embedding.map(e => e[0]);
                y = embedding.map(e => e[1]);
    ...
    _computePCA(matrix, nComp = 5) {
        ...
        const nc = Math.min(nComp, Math.min(n, p));
        // Center the data
        ...
        if (p > n) {
            ...
            const { eigenvalues, eigenvectors } = this._powerIteration(gram, nc);
            const totalVar = eigenvalues.reduce((a, b) => a + Math.abs(b), 0) || 1;
            // Scores: [nc][n] → transpose to [n][nc]
            const scoresT = Array.from({ length: n }, (_, i) =>
                eigenvectors.map((ev, k) => ev[i] * Math.sqrt(Math.abs(eigenvalues[k])))
            );
            // Loadings: [p][nc]
            const loadings = Array.from({ length: p }, () => new Array(nc).fill(0));
            for (let k = 0; k < nc; k++) {
            ...
            return { scores: scoresT, loadings, explained, eigenvalues };
        } else {
            ...
            const { eigenvalues, eigenvectors } = this._powerIteration(cov, nc);
            const totalVar = eigenvalues.reduce((a, b) => a + Math.abs(b), 0) || 1;
            const scores = Array.from({ length: n }, (_, i) =>
                eigenvectors.map(ev => {
                    let s = 0;
                    for (let j = 0; j < p; j++) s += centered[i][j] * ev[j];
                    return s;
                })
            );
            const loadings = Array.from({ length: p }, (_, j) => eigenvectors.map(ev => ev[j]));
            const explained = eigenvalues.map(e => e / totalVar);
            return { scores, loadings, explained, eigenvalues };
        }
    }
    ...
    _populateComponentSelectors(nComponents, prefix, explained) {
        ...
        for (let i = 0; i < nComponents; i++) {
            const label = explained
                ? `${prefix}${i + 1} (${(explained[i] * 100).toFixed(1)}%)`
                : `${prefix} ${i + 1}`;
        ...
        selX.value = '0';
        selY.value = '1';
        document.getElementById('clbUmapComponentRow').style.display = 'flex';
        // Show loadings toggle for PCA
        const loadingsToggle = document.getElementById('clbUmapLoadingsToggle');
        loadingsToggle.style.display = prefix === 'PC' ? '' : 'none';
    }

    _onComponentChange() {
        if (!this._clbUmapData || !this._clbUmapAllScores) return;
        const compX = parseInt(document.getElementById('clbUmapCompX').value);
        const compY = parseInt(document.getElementById('clbUmapCompY').value);
        const scores = this._clbUmapAllScores;
        const method = this._clbUmapData.method;
        const explained = this._clbUmapAllExplained;

        const x = scores.map(s => s[compX]);
        const y = scores.map(s => s[compY]);

        let axisLabels;
        if (method === 'pca' && explained) {
            axisLabels = [
                `PC${compX + 1} (${(explained[compX] * 100).toFixed(1)}%)`,
                `PC${compY + 1} (${(explained[compY] * 100).toFixed(1)}%)`
            ];
        } else {
            axisLabels = [`UMAP ${compX + 1}`, `UMAP ${compY + 1}`];
        }

        this._clbUmapData.x = x;
        this._clbUmapData.y = y;
        this._clbUmapData.axisLabels = axisLabels;
        this._clbUmapData._compX = compX;
        this._clbUmapData._compY = compY;

        const { cellLines, categories, mutStatus, splitGene } = this._clbUmapData;
        const colorBy = document.getElementById('clbUmapColorBy').value;
    ...
    _addLoadingsArrows(plotDiv) {
        ...
        const mags = geneNames.map((g, i) => ({
            ...
            lx: loadings[i][compX],
            ly: loadings[i][compY],
            mag: Math.sqrt(loadings[i][compX] ** 2 + loadings[i][compY] ** 2)
        }));
